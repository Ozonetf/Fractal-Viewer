...
{
    HRESULT res = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, m_d2dFactory.GetAddressOf());
    ThrowIfFailed(res);
}
...
{
    //required flag for D2D and D3D interoperability 
    ...
    //Set D2D device context with DXGI device
    Microsoft::WRL::ComPtr<IDXGIDevice3> DGXIdevice;
    DX::ThrowIfFailed(
        m_d3dDevice.As(&DGXIdevice)
    );
    DX::ThrowIfFailed(
        m_d2dFactory->CreateDevice(DGXIdevice.Get(), &m_d2dDevice)
    );
    DX::ThrowIfFailed(
